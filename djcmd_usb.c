...
{
    struct stat ss, ds;
    if (stat(src, &ss) != 0) return 0;       /* src missing — nothing to copy */
    if (stat(dst, &ds) != 0) return 1;       /* dst absent — must copy */
...
void usb_get_machine_id(char *out, size_t sz)
{
    ...
    if (!home) { strncpy(out, "unknown", sz - 1); return; }
    ...
    snprintf(out, sz, "%08lx%08lx%08lx",
...
int usb_scan(USBDevice *out, int max)
{
    ...
    while (fgets(line, sizeof(line), f) && count < max) {
        ...
        char marker[768];
        snprintf(marker, sizeof(marker), "%s/%s", mountpoint, DJCMD_USB_MARKER);
        struct stat st;
        if (stat(marker, &st) != 0)
            continue;
        ...
        strncpy(dev->mount_point, mountpoint, sizeof(dev->mount_point) - 1);
        ...
        if (d) {
            ...
            while ((ent = readdir(d))) {
                ...
                if (realpath(link_path, resolved_label) &&
                    ...
                    if (strcmp(resolved_label, resolved_dev) == 0) {
...
void usb_crate_read_origin(const char *crate_path, char *out, size_t sz)
{
    ...
    while (fgets(line, sizeof(line), f)) {
        if (line[0] != '#') break;                   /* headers are at the top */
                     ...
                     char *status_out, size_t status_sz)
{
    ...
    while (fgets(line, sizeof(line), src_crate)) {
        ...
        /* ── Audio file ── */
        char usb_audio[2048];
        snprintf(usb_audio, sizeof(usb_audio), "%s/%s/%s",
                 usb_mount, DJCMD_USB_MUSIC, rel);
        ...
        if (last_slash) { *last_slash = '\0'; usb_mkdir_p(parent); }

        if (file_needs_copy(abs_path, usb_audio)) {
            if (copy_file(abs_path, usb_audio) == 0) copied++;
            else                                      failed++;
        } else {
            skipped++;
        }
        ...
        char sidecar_src[2056], sidecar_dst[2056];
        snprintf(sidecar_src, sizeof(sidecar_src), "%s.djcmd", abs_path);
        snprintf(sidecar_dst, sizeof(sidecar_dst), "%s.djcmd", usb_audio);
    ...
    snprintf(status_out, status_sz,
             "Exported '%s': %d copied, %d skipped, %d failed",
             export_name, copied, skipped, failed);
    return 0;
}
...
int usb_eject(const char *mount_point)
{
    ...
    if (f) {
        ...
        while (fgets(line, sizeof(line), f)) {
            if (sscanf(line, "%511s %511s %255s", dev, mp, rest) == 3 &&
    ...
    if (block_dev[0]) {
        snprintf(cmd, sizeof(cmd),
                 "udisksctl power-off -b \"%s\" 2>/dev/null", block_dev);
        system(cmd);
    }
